...
public class FilterBuilder {
    ...
    public FilterBuilder and(FilterCriteria filterCriteria) {
        ...
        if (this.filterCriteria == null) {
            throw new IllegalStateException("And operator cannot be called on one filterCriteria. It should be chained.");
        }
    ...
    public FilterBuilder or(FilterCriteria filterCriteria) {
        ...
        if (this.filterCriteria == null) {
            throw new IllegalStateException("Or operator cannot be called on one filterCriteria. It should be chained.");
        }
    ...
    public FilterBuilder in(String key, List<Integer> value) {
        if (value.size() == 0) {
            throw new IllegalStateException("In filter requires at least one value.");
        }
    ...
    public FilterBuilder range(String key, Integer startValue, Integer endValue) {
        if (startValue == null || endValue == null) {
            throw new IllegalStateException("Range filter Requires 2 values for min and max values.");
        }
        if (startValue > endValue) {
            throw new IllegalStateException("Start Value cannot be greater than end value in range filter.");
        }
